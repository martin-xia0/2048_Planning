...
class Game:
    ...
    def move_and_score(self, direction):
        '''
        direction:
            0: left
            1: down
        ...
        board_to_left = np.rot90(self.board, -direction)
        for row in range(self.size):
            core, score_row = _merge_and_score(board_to_left[row])
            score += score_row
            board_to_left[row, :len(core)] = core
            board_to_left[row, len(core):] = 0
        ...
        # rotation to the original
        self.__board = np.rot90(board_to_left, direction)
        self.score_move += score
        self._maybe_new_entry()
        return score

    def only_move(self, direction):
        '''
        direction:
            0: left
            1: down
